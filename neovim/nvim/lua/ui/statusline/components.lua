...
local hl_str = tools.hl_str
local get_hl_hex = tools.get_hl_hex
...
local function get_theme_color(mode)
  if vim.g.colors_name == 'ice-cave' then
    return mode == "insert" and "#4b8b51" or "#9580FF"
  end
...
M.colors = {
  ...
  bg_hl = get_hl_hex("Normal").bg and tools.lighten(get_hl_hex("Normal").bg, 0.93) or "none",
}
...
function M.reload_colors()
  ...
  -- reload other colors
  M.colors.bg_hl = get_hl_hex("Normal").bg and tools.lighten(get_hl_hex("Normal").bg, 0.93) or "none"
...
function M.fileinfo(opts)
  ...
  local dir = tools.pretty_dirpath()()
